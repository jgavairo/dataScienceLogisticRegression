...
def load_predictions(predictions_path='output/houses.csv'):
    ...
    try:
        ...
        print(f"✓ Predictions loaded: {len(predictions)} examples")
        return predictions
    except FileNotFoundError:
        print(f"❌ Error: File {predictions_path} not found")
...
def load_true_labels(dataset_path='datasets/dataset_truth.csv'):
    ...
    try:
        ...
        if 'Index' not in true_labels.columns or 'Hogwarts House' not in true_labels.columns:
            print(f"❌ Error: File must contain columns 'Index' and 'Hogwarts House'")
            sys.exit(1)
        
        # Remove rows without house (if any)
        true_labels = true_labels.dropna(subset=['Hogwarts House'])
        
        print(f"✓ True labels loaded: {len(true_labels)} examples")
        return true_labels
    except FileNotFoundError:
...
def calculate_accuracy(predictions_df, true_labels_df):
    ...
    if len(merged) == 0:
        print("❌ Error: No match between predictions and true values")
        sys.exit(1)
...
def print_results(results):
    ...
    print(f"   → {results['correct']} / {results['total']} correct predictions")
    
    # Per-class metrics
    print("\n" + "-"*80)
    print("📈 METRICS PER HOUSE:")
    print("-"*80)
    ...
    if results['accuracy'] >= 0.98:
        ...
        print("   👍 GOOD! Your model is decent but could be improved.")
    elif results['accuracy'] >= 0.80:
        print("   ⚠️  ACCEPTABLE. Consider adjusting hyperparameters.")
    else:
        print("   ❌ WEAK. The model needs significant improvements.")
    ...
    print(f"\n   📉 Most difficult house: {worst_house[0]} (F1={worst_house[1]['f1_score']:.2%})")
    print(f"   📈 Best predicted house: {best_house[0]} (F1={best_house[1]['f1_score']:.2%})")
